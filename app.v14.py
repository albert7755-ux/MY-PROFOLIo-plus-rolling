        ...
        with st.spinner('正在進行 AI 運算 (含蒙地卡羅模擬)...'):
            ...
            try:
                ...
                with col_top2:
                    ...
                    r2c2.metric("最大回撤 (MDD 淨值)", f"{mdd:.2%}", delta_color="inverse")

                    # ==========================================
                    # ★ 新增：總部位 MDD vs 淨值 MDD 對照表
                    # ==========================================
                    if use_margin:
                        # 總部位序列 = 標準化價格 × 槓桿倍數（分母放大，反映標的本身風險）
                        total_position_val = raw_port_val * leverage
                        mdd_total_position = calculate_mdd(total_position_val)
                        mdd_no_margin = calculate_mdd(raw_port_val)

                        st.markdown("---")
                        st.subheader("🔬 MDD 雙視角對照（融資分析）")

                        col_mdd1, col_mdd2, col_mdd3 = st.columns(3)

                        col_mdd1.metric(
                            "無融資 MDD（標的本身）",
                            f"{mdd_no_margin:.2%}",
                            delta_color="inverse",
                            help="以自有本金買入，不加槓桿的最大回撤。反映標的本身的市場風險。"
                        )
                        col_mdd2.metric(
                            "總部位 MDD（加槓桿後）",
                            f"{mdd_total_position:.2%}",
                            delta=f"vs 無融資：{mdd_total_position - mdd_no_margin:+.2%}",
                            delta_color="inverse",
                            help="以總部位（含借款）為分母計算。理論上與無融資相同，差異來自利息成本的微小影響。"
                        )
                        col_mdd3.metric(
                            "淨值 MDD（客戶視角）",
                            f"{mdd:.2%}",
                            delta=f"vs 無融資：{mdd - mdd_no_margin:+.2%}",
                            delta_color="inverse",
                            help="以自有本金（淨值）為分母計算。反映客戶口袋的實際損失深度，槓桿越高、MDD 越大。"
                        )
...
| 視角 | 分母 | 用途 |
|------|------|------|
| 總部位 MDD | 總持倉（含借款） | 評估「標的本身」的風險，不受槓桿影響 |
| 淨值 MDD | 自有本金（淨值） | 評估「客戶實際損失」，銀行風控 / 爆倉判斷用 |

> 融資成數 {loan_ratio:.0%}、槓桿 {leverage:.1f} 倍 → 淨值 MDD 約為無融資的 **{mdd/mdd_no_margin:.1f} 倍**（另含利息成本侵蝕）
                            """
                        )
